  ...
  const submitHandler = async () => {
    ...
    for (let i = 0; i < stateArr.length - 1; i++) {
      if(stateArr[i][1] === "checked"){
        updatedUser = await axios.patch(`http://localhost:7000/users/${user._id}/lists/${stateArr[i][0]}`, {stock: stockSymbol.toUpperCase()})
      }
    }
    console.log(stateArr[stateArr.length - 1])
    if(stateArr[stateArr.length - 1][1] === "checked"){
      updatedUser = await axios.patch(`http://localhost:7000/users/${user._id}/lists/${stateArr[stateArr.length - 1][0]}`, {stock: stockSymbol.toUpperCase()})
    }
